            ...
            if line.startswith('#') or line.lstrip() == "":
                ...
                if len(lineList) == 6:
                    ...
                    if scope == 'ALL':
                        ...
                        logger.info('Files that start with [%s] are subject to delete/initialize.', prefix)
                    else:
...
with open(configFile, 'r') as cf:
    for line in cf:
        if line.startswith('#') or line.lstrip() == '':
            ...
            method = str(lineList[5].rstrip())
            logger.info('[MODE:%s] [DIR:%s] [SCOPE:%s] [PREFIX:%s] [TARGET TIME:%i days] [METHOD:%s]', mode, work_dir, scope, prefix, target_date, method)
            logger.info('-' * 70)
